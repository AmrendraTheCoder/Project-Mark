          ...
          <nav className="flex flex-col gap-3 px-3 py-6">
            ...
            <Collapsible>
              <CollapsibleTrigger
                ...
                className={`flex items-center w-full mb-2 p-3 space-x-2 rounded-md hover:bg-blue-600 ${
                  active.inventory ? "bg-blue-800" : "bg-slate-800"
                } text-slate-50 justify-between`}
              >
                ...
                {inventoryLinks.map((item, i) => {
                  return (
                    <Link
                      href={item.href}
                      key={i}
                      className={`flex items-center justify-between p-2.5 pr-6 mb-2 rounded-lg hover:bg-blue-500 transition-all duration-300 ${
                        active[item.title.replace(" ", "")] ? "bg-blue-600" : ""
                      }`}
                      onClick={() =>
                        handleItemClick(item.title.replace(" ", ""))
                      }
                    >
                      <span className="pl-8 font-medium text-sm">
                        {item.title}
                      </span>
